...
// i18n: nokta metinleri fp.* ile paylaşılır; tip değeri BE'de TR (olcusel/nitel).
...
export async function viewHourlyPoints(container) {
  ...
  const table = new DataTable(container, {
    ...
    columns: [
      ...
      { label: () => t('field.measureLocation'), key: 'measureLocation' },
  ...
  function openForm(row) {
    ...
    openDrawer({
      ...
      fields: [
        ...
        { name: 'measureLocation', label: () => t('field.measureLocation'), type: 'text', required: true },
  ...
  async function remove(row) {
    const ok = await confirmDialog({ title: t('fp.deleteTitle'), body: t('fp.deleteBody', { name: row.measureLocation }), confirmLabel: t('action.delete'), danger: true });
    if (!ok) return;
