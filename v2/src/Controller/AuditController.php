...
final class AuditController
{
    ...
    public function show(int $id): never
    {
        ...
        if ($row === null) {
            Response::fail(404, 'Denetim sorusu bulunamadi');
        }
        Response::ok(Audit::fromRow($row));
    }
    ...
    public function store(array $input): never
    {
        ...
        $v = (new AuditValidator())->validate($input, isCreate: true);
        if ($v->fails()) {
            Response::invalid($v->errors());
        }

        $id = $this->repo->create(Audit::toColumns($input));
        Response::created(Audit::fromRow($this->repo->find($id)));
    }
    ...
    public function update(int $id, array $input): never
    {
        ...
        $v = (new AuditValidator())->validate($input, isCreate: false);
        if ($v->fails()) {
            Response::invalid($v->errors());
        }

        try {
            $this->repo->update($id, Audit::toColumns($input), $input['updatedAt'] ?? null);
        ...
        Response::ok(Audit::fromRow($this->repo->find($id)));
    }
