...
use App\Repository\FirstOffPointRepository;
use App\Validator\FirstOffPointValidator;
use RuntimeException;

/**
 * Yetki + dogrulama + yanit. Is mantigi Repository'de, SQL burada YOK.
 */
final class FirstOffPointController
{
    private FirstOffPointRepository $repo;

    public function __construct(private Context $ctx)
    {
        $this->repo = new FirstOffPointRepository($ctx);
    }
    ...
    public function index(array $query): never
    {
        ...
        Response::ok(
            FirstOffPoint::fromRows($result['rows']),
    ...
    public function show(int $id): never
    {
        ...
        Response::ok(FirstOffPoint::fromRow($row));
    }
    ...
    public function store(array $input): never
    {
        ...
        $v = (new FirstOffPointValidator())->validate($input, isCreate: true);
        if ($v->fails()) {
            Response::invalid($v->errors());
        }

        $id = $this->repo->create(FirstOffPoint::toColumns($input));
        Response::created(FirstOffPoint::fromRow($this->repo->find($id)));
    }
    ...
    public function update(int $id, array $input): never
    {
        ...
        $v = (new FirstOffPointValidator())->validate($input, isCreate: false);
        if ($v->fails()) {
            Response::invalid($v->errors());
        }

        try {
            $this->repo->update($id, FirstOffPoint::toColumns($input), $input['updatedAt'] ?? null);
        ...
        Response::ok(FirstOffPoint::fromRow($this->repo->find($id)));
    }
