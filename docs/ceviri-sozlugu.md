# Arayüz çeviri sözlüğü — TR / EN

Kaynak: `Ozmel_Order_Tracking_Excel.xlsx` — müşterinin (Mosdorfer) kendi kullandığı
terimler. Sipariş/malzeme alanlarında bu tablo esas alınmalı; müşteri ekrana
baktığında tanıdık kelimeleri görmeli.

> Excel'de iki yazım hatası var: `Manifacturer` → **Manufacturer**,
> `Remaning` → **Remaining**. Doğrusu kullanılacak.

---

## 1. Sipariş ve malzeme alanları (Excel'den birebir)

| Türkçe | English |
|---|---|
| Üretici | Manufacturer |
| Müşteri | Customer |
| Sipariş Numarası | Purchase Order Number |
| Sipariş Sekmesi | Purchase Order Section |
| Sipariş Günü / Tarihi | Purchase Order Date |
| Malzeme Numarası / Kodu | Material Number |
| Revizyon Numarası | Rev. Number |
| Malzeme Adı | Material Name |
| Hammadde | Raw Material |
| Sipariş Adedi | Order Quantity (Pcs) |
| Sevk Edilen Adet | Shipped Quantity (Pcs) |
| Kalan Adet | Remaining Quantity (Pcs) |
| İstenen Sevk Tarihi | Requested Ship Date |
| Sevkiyat Günü | Ship Date |
| Sevkiyat Adedi | Ship Quantity (Pcs) |
| Sevkiyat Yeri | Ship To |
| Ne ile Sevk | Shipment Type |
...
| Tedarikçi & Kalite | Supplier & Quality |
| Tedarikçi & Site | Supplier & Site |
| Giriş Kalite Kontrolleri | Incoming Inspections |
| First-Off Noktaları | First-Off Points |
| First-Off Kayıtları | First-Off Records |
| Saatlik Noktalar | Hourly Points |
| Saatlik Kayıtlar | Hourly Records |
...
| Üretim Planı | Production Plan |
| İş Emirleri | Work Orders |
| Üretim Girişi | Production Entry |
| Üretim Raporu | Production Report |
| Üretim Panosu | Production Board |
| Verimlilik | Productivity |
| Ürün Ağaçları | Product Trees |
| Satınalma & Stok | Purchasing & Stock |
| Satınalma İstekleri | Purchase Requests |
| Satınalma Girişleri | Purchase Receipts |
| Stok Durumu | Stock Status |
| Satış | Sales |
| Satış Siparişleri | Sales Orders |
| Üretim Siparişleri | Production Orders |
...
| Kullanıcılar | Users |
| Makine Planı | Machine Plan |

---

## 3. Eylemler ve ortak arayüz
...
| Türkçe | English |
|---|---|
| Hammadde Bekleniyor | Awaiting Raw Material |
| Üretimde | In Production |
| Kalite Kontrolde | In Quality Control |
| Sevke Hazır | Ready to Ship |
| Kısmi Sevk | Partially Shipped |
| Sevk Edildi | Shipped |
| İade | Returned |
...
## 5. Üretim ve kalite alanları

| Türkçe | English |
|---|---|
| İş Emri No | Work Order No |
| Hedef Miktar | Target Quantity |
...
| Kapasite (Vardiya) | Capacity (per Shift) |
...
| Uygun | Pass |
| Uygunsuz | Fail |
| Kontrol Noktası | Control Point |
| Sicil No | Employee No |
| Yetkinlik | Skill |
| Tedarikçi | Supplier |
| Parti No | Lot No |
| Miktar | Quantity |
| Birim | Unit |
| İstek Tarihi | Request Date |
...
| Bu kayıt başkası tarafından değiştirildi | This record was modified by someone else |
| Oturum süresi doldu — lütfen tekrar giriş yapın | Session expired — please sign in again |
| Kullanıcı adı veya şifre hatalı | Incorrect username or password |
| Silmek istediğinize emin misiniz? | Are you sure you want to delete this? |

---

## Notlar

- **Tarih biçimi**: TR `GG.AA.YYYY`, EN `DD/MM/YYYY`. Excel `YYYY-MM-DD` kullanıyor;
  ISO biçimi iki dilde de kabul edilebilir, karar verilmeli.
- **Sayı biçimi**: TR ondalık ayracı virgül (`2,09`), EN nokta (`2.09`).
  Excel'de EN sayfaları nokta kullanıyor.
- **Veri içeriği çevrilmiyor.** İş merkezi adları, operasyon adları, malzeme
  adları veritabanından geldiği gibi gösterilir. Operasyon adları için `terms`
  tablosu var (40 çeviri kaydı: "Cap Plugging" → "Kapak Takma"); dil EN iken
...
Modül anahtarları (özet): `pc.*` Kod Tanımları (secDrawing/secMeasures/secStock/secRel, minStock, supplyDays, boxQty, suppliers, outgoingOp, parentCode…), `tp.*` Görev Kişileri, `wh.*` Çalışma Saatleri (start/breakStart/breakEnd/end, net/break/lunch, help, hoursShort…), `cap.*` Kapasiteler, `opr.*` Operatörler (skills, skillsHelp…), `rt.*` Rotalar. Tümü `public/js/core/i18n.js` DICT'te birebir mevcuttur.

**Rotalar yeniden yazımı (referans v78 `viewRoutes` düzeni — ürün seçici + zaman çizelgesi):** ekran artık solda ürün listesi, sağda seçili ürünün `sequence` bazlı adım çizelgesi. Aynı sırada birden çok iş merkezi = Aktif/Alternatif satırı. Alt operasyon ondalık sırayla (1 → 1.1) eklenir; varyant seçenekleri aynı (ürün, sıra, operasyon) üçlüsündeki tüm alternatiflere birlikte uygulanır. Yeni `rt.*` anahtarları: stepCount, cncGroup, alt, footerNote, addAltOp, addVariant, editVariant, variantFallback, activeHelp, defineHint, altOpTitle, altOpHint, altOpAdded, variantTitle, variantHint, variantGroupName, variantGroupPlaceholder, variantOptionsSemicolon, variantOptionsPlaceholder, variantLabelRequired, variantOptionsRequired, variantRemove, variantRemoveTitle, variantRemoveBody, variantSaved, variantRemoved. Kaldırılan (eski tablo/TagList düzeni): addOp, variantsCol, noVariants, variantLabel, variantOptions, variantsHelp, variantPlaceholder. Varyant seçenekleri artık **noktalı virgülle** ayrılan metin (virgül ondalık ayracı olduğu için TagList değil).

**Rotalar v2 tasarım hizalaması (`docs/Rotalar-v2.dc.html`):** düzen `.rt-root` altında flex-doldurmalı iki kolon (sol: arama + ürün listesi; sağ: panel + alt not); yükseklik flex ile bağlanır, paylaşılan `.content`'e dokunulmaz (önceki `.content:has` denemesi ana içerik/menü akışını bozuyordu). Görsel: sıra kutusu 30×30 (ana=dolu aksan/beyaz, alt op=beyaz/hairline), operasyon başlığı 18px, Düzenle/Sil çerçeveli (tasarımdaki gibi), panel başlığı BÜYÜK HARFE ÇEVRİLMEZ. Yeni `rt.*` anahtarları: summary (yeniden ifade), panelSummary, noActiveWarn(+Post), noMatchTitle, clearSearch. Aktif hat uyarısı bağlantısı `cap.title` (Kapasite Yönetimi). Kapasiteler aynı `.part-picker/.tl-*` tabanını kullanır; tüm Rotalar stilleri `.rt-root` altına kapsandığından etkilenmez.

**Kapasiteler yeniden yazımı (referans v78 `viewCapacity` düzeni — Rotalar'daki ürün seçici + zaman çizelgesi + üç ek bölüm):** solda ürün listesi (üçüncü satır `Hedef: <darboğaz>/gün`), üstte **Makine Durumu** (açık iş emirlerinden, salt okunur) ve **Veri Kontrolü Uyarıları** (duplicate/orphan/missing, ≤30), sağda üç KPI kartı + kapasite çizelgesi. Kapasite artık `(ürün, iş merkezi, operasyon)` üçlüsünde (migration 032: `operation_id` + tekil anahtar). Aktif hat seçimi (radyo) **burada** yapılır → `routes.is_active` güncellenir. Dakika/adet girilen CNC adımlarında kapasite Çalışma Saatleri'nden canlı hesaplanır (`csOzet`). Yeni `cap.*` anahtarları: machineStatus(+Sub), remaining, warnings, warnDuplicate/Orphan/Missing (+Msg), activeSuffix, goToProduct, deleteRecord, target(+None), kpiTarget(+Foot/None), kpiBottleneck(+Foot), kpiUndefined(+Some/None), bottleneckBadge, activeLine, perDay, perDayPlaceholder, minPlaceholder, minHelp(+NoHours), footerNote, activeUpdated, capUpdated, capDeleted, minUpdated, emptyRoutes(+Msg). ETL: `capacity.operasyon → operation_id`, `minutes ← dakikaPerAdet` (eski eşleme `dakika`'yı okuyordu — hep NULL kalıyordu).

**Makine Durumu paneli yeniden yazımı (`docs/Makine-Durumu.md`, Kapasiteler ekranında kalır):** salt-okunur bölüm artık **katlanabilir** (varsayılan KAPALI, durum `localStorage: ozmel.cap.machineStatus.collapsed`), başlıkta özet `N iş merkezi · M açık iş` + "Salt okunur" rozeti. Açıkken sıralama/açıklama şeridi (yüke/ada göre) + `minmax(322px,1fr)` kart ızgarası; her kartta yük çubuğu (pay > 0,60 → `--color-warning`, aksi `--color-accent`), toplam kalan (mono, `tr-TR`), ilk 5 satır + "+N daha" ile satır içi genişletme. Satır spec'in 3 kolonunu korur (kod · operasyon · kalan); **iş emri no** (woNo, aynı ürün+operasyon tekrarını ayırt eder) kod hücresinde küçük ön-ek olarak gösterilir. Satır `#work-orders?id=<woId>` hash bağlantısıdır. Veri istemcide `listAll()`'dan türetilir (yeni BE ucu yok); 5 dk'lık yenileme ve yükleme/hata durumları uygulanmadı. Yeni `cap.*` anahtarları: machineStatusReadonly, machineSummary, machineSortTotal, machineSortName, machineRowHint, machineTotalRemaining, machineOps, machineMoreOps, machineLess, machineEmpty(+Msg). Kaldırılan CSS: `.cap-machines/.cap-machine*` → `.cap-mstat*`.

**Ortak katlanabilir panel bileşeni (`public/js/core/collapsible.js`, `.cpanel*`):** kutu + tıklanır başlık (caret, başlık, `metaHTML` özet, sağa yaslı `rightHTML` rozet) + tembel `body`. Katlama durumu `localStorage: ozmel.collapse.<key>` (varsayılan kapalı); `onToggle` ile çağıran modül yeniden çizilir. Blueprint kutusu (neutral-400 kenar + dört köşe aksan registration işareti) ve başlık stilleri artık `.cpanel*`; Makine Durumu'na özel ızgara/kart/satır sınıfları `.cap-mstat*` olarak kaldı. **Makine Durumu** ve **Veri Kontrolü Uyarıları** panelleri bu bileşeni paylaşır. Veri Kontrolü Uyarıları artık katlanabilir (varsayılan kapalı, başlıkta tür kırılımı `2 çakışma · 1 rotasız · 3 eksik` + toplam sayı rozeti; uyarı yoksa panel hiç render edilmez). Yeni `cap.*` anahtarları: warnCountDup, warnCountOrphan, warnCountMissing.

**Üretim Planı yeniden yazımı (`docs/Uretim-Plani.md`, v78 `viewUretimPlani` düzeni):** satır içi düzenlenen haftalık ızgara — satır = makine (`routes.work_center` distinct), kolon = 7 gün, her hücre TEK plan (migration `033_machine_plan_unique`: `(tenant, date, work_center_id)` tekil). Hücre: `<select>` (Açık İş Emirleri: `IE-{woNo} — {kod} ({kalan} kalan)` / Ürün plansız: kod, alfabetik) + hedef `<input>` + `IE-{woNo}` notu. Kaydetme satır içi **upsert**: `MachinePlanRepository.create` `(date, wc)` varsa günceller (ikinci kayıt açılmaz), yoksa oluşturur; seçim `—` → sil. Hedef ön dolumu paylaşılan `core/capacity.js` (`resolveCapacity`/`netWorkMinutes` — `minutes` doluysa çalışma saatlerinden canlı, yoksa `capacity_per_shift`; `(ürün, iş merkezi, operasyon)`). Kapasite aşımı: kırmızı alan + `kapasite {n}` notu, kayıt engellenmez. Gruplama: İş Merkezine/Operasyona Göre (operasyon grup başlıkları katlanabilir, durum `localStorage: ozmel.mp.collapsedOps`). Operasyon sırası sabit: `Cutting → Countersink → Marking → Pressing → Packaging`, kalanlar (TR dahil) alfabetik. Üç özet: İş Merkezi / **Operasyon** / Ürün bazlı (spec'teki "Makine" bu şemada iş merkeziyle aynı olduğundan Operasyon'a alındı). Çakışma sadeleştirildi: mevcut 409 STALE + "Yeniden yükle" (değiştiren kişi/saat ve "Farkları göster" YOK — `updated_by` doldurulmuyor). Yeni `mp.*` anahtarları: prevWeek, nextWeek, thisWeek, gridTitle, onlyPlanned, byWorkCenter, byOperation, noMachines(+Msg), goRoutes, grpOpenWo, grpProduct, remainingWord, machineWord, opNone, overCap, readOnly, sumWorkCenter, sumOperation, sumProduct, weekTotal, noPlansYet. Kaldırılan (eski hücre-diyalog/drawer düzeni): mp.new, newTitle, editTitle, cellEmpty, target, addPlan, selectWo, updated, added, deleted, deleteTitle, deleteBody.

**Genel Üretim Raporu (yeni ekran, `docs/uretim-raporu-brief.md`, referans v78 `viewUretimRaporu`):** salt-okunur pano; dönem (günlük/haftalık/aylık) + gezinme, 4 KPI (üretilen/fire oranı/genel gerçekleşme/toplam duruş — eşiğe göre renk), otomatik "Dikkat Edilmesi Gereken Noktalar", duruş nedeni **Pareto**'su (kümülatif ≤%80 danger), makine/ürün özetleri, günlük trend (çubuk=gerçek, dikey çizgi=plan). Veri istemcide türetilir; plan `machine_plans.target_quantity`, gerçek `production.actual_quantity`, eşleştirme (tarih, iş merkezi) → `production→work_order→work_center`. Migration `035_downtime_reasons`: `downtime_reasons` tablosu + `production.downtime_reason_id` + 5 başlangıç nedeni. **Duruş Nedenleri** (Tanımlar, `dtr.*`): ad + aktif/pasif CRUD; production'da kullanılınca silinemez (IN_USE), pasife alınır. **Üretim Girişi**'ne duruş alanları (`prod.downtimeStart/End/Reason`): opsiyonel, biri girilirse diğeri de (BE doğrular). Duruş süresi = (bitiş−başlangıç) − mola kesişimi (sabah/öğle arası/öğleden sonra molası — `working_hours`), paylaşılan `core/capacity.js:downtimeMinutes`. Paylaşılan `core/format.js`: fmtTr/fmtDuration/fmtDateTR/fmtPct. Yeni `ur.*`, `dtr.*`, `prod.downtime*`, `menu.uretim-raporu`, `menu.downtime-reasons` anahtarları.

**Üretim Panosu (yeni ekran, `docs/uretim-panosu-brief.md`, referans v78 `viewUretimPanosu`):** sahaya asılan TV ekranı — diğerlerinden iki farkı: (1) SALT görüntüleme (form yok), (2) uygulamadaki TEK otomatik yenilenen modül (varsayılan **15 sn**; eski veri ekranda kalır, iskelet yok; hata sessizce geçilir; modülden çıkınca `clearInterval` — kalıcı `.pano-root` DOM'da kalıp kalmamasına bakılır). Uzaktan okunsun diye tipografi büyük (başlık 46 / KPI 58 / kart 44 px). Başlık şeridi: `pano.title` + tarih (gün adı + GG.AA.YYYY) · sağda **canlı saat** (mono 40px, `toLocaleTimeString("tr-TR")`, 1 sn — yalnız saat öğesi güncellenir) + `pano.fullscreen` (kök öğeye `requestFullscreen`/`exitFullscreen`, hata yutulur). 4 KPI (`pano.kpiPlanned/kpiActual/kpiOverall/kpiDowntime`). **Renk eşiği panoda FARKLI** — `--pano-iyi/orta/kotu(-zemin)` tokenları, iyi/orta arası **30 puan** (diğer ekranlarda 20; uzaktan daha az kırmızı); `core/report.js:thresholdClass` DEĞİL. İş merkezi kartları `minmax(340px,1fr)` ızgara, iş merkezi adına göre sıralı (`localeCompare "tr"`). Bugün planı yoksa `pano.emptyTitle`/`pano.emptyBody`. `pano.thisWeek` şeridi: Pzt–Paz 7 kolon, bugünün kolonu accent zemin/çerçeve. Veri istemcide türetilir (yeni BE ucu yok): hedef `machine_plans.target_quantity`, gerçekleşen `production.actual_quantity` **(tarih, iş emri)** eşleşmesi, duruş `core/capacity.js:downtimeMinutes` (mola düşülmüş). Tarih hesabı YEREL (`core/report.js` `fmtISO`/`mondayOf`/`addDays`, hafta başı Pazartesi). Yeni `pano.*` + `menu.uretim-panosu` anahtarları.

**Verimlilik (yeni ekran, `docs/verimlilik-brief.md`):** salt-okunur pano; tek soruya odaklı — planladığımızı üretebildik mi? Üretim Raporu ile aynı iskeleti paylaşır: dönem şeridi (günlük/haftalık/aylık + gezinme), renk eşiği (≥%90 success · ≥%70 warning · altı danger), KPI kartı — üçü de `core/report.js`'e çıkarıldı (`createPeriodStrip`/`thresholdClass`/`kpiCard`, şerit etiketleri `ur.*` ile ortak); `uretimRaporu.js` bu ortak parçaları kullanacak şekilde davranış değişmeden refactor edildi. **Satır birimi:** `machine_plans` kaydı — yalnızca `work_order_id` DOLU olanlar ("plansız ürün" planları girmez). 4 KPI (toplam planlanan/gerçekleşen/genel gerçekleşme/toplam duruş) + tek tablo (İş Emri Bazında Gerçekleşme: tarih · makine · ürün/iş emri · planlanan · gerçekleşen · % · duruş). Gerçekleşen/duruş **(tarih, iş emri)** üzerinden eşleşir (sadece iş merkezi değil — aynı makinede gün içinde iş emri değişebilir): gerçekleşen = `production.actual_quantity` toplamı, duruş = `core/capacity.js:downtimeMinutes` toplamı. Satır tıklanınca `#work-orders?id=` ile iş emri salt görüntü açılır. Boş hal: dönemde iş emrine bağlı plan yoksa tablo/KPI yerine `vr.emptyTitle`/`vr.emptyBody`. Dışa aktarma ve otomatik uyarılar bu ekranda YOK. Yeni `vr.*` + `menu.verimlilik` anahtarları.

**Stok Durumu (yeni ekran, `docs/stok-durumu-brief.md`, referans v78 `viewStok`):** salt görüntüleme; iki bölüm. **Bölüm 1 — Hammadde Stok Durumu:** `product_codes` tip='Hammadde' satırları; ONAYLI GELEN · TÜKETİLEN (KESİM) · NET STOK (kg) · DURUM · BAĞLI SİPARİŞ(LER). Onaylı gelen = `purchase_receipts.quantity`, YALNIZCA `incoming_inspections.overall_result='Uygun'` (`purchase_receipt_id` eşleşmesi) olan girişler — **Şartlı Kabul SAYILMAZ** (Melih'e sorulacak). Tüketilen (kesim) = kesim operasyonundaki `production.actual_quantity` × ürün ağacı çarpanı × `material_weight`. Kesim operasyonu: adı `kesim`/`cutting` içeren `operations`; yoksa hammadde `outgoing_operation_id`. Adet eşd. = kg / `material_weight`; **ağırlık yoksa `—`** (14/57 üründe var). NET renk: negatif danger, değilse success. DURUM yalnız sorun varsa: netKg<0 → `stok.negative` (danger), netKg<`min_stock_level` → `stok.low` (warning, title min); `min_stock_level` hiç girilmemiş → başta boş. Bağlı sipariş: `purchase_requests.order_id`→`orders.order_no` (virgülle; yoksa `stok.generalStock`). Malzeme kodu `#product-codes?id=`, sipariş no `#orders?id=` çapraz bağlantı. **Ürün ağacı (BOM) bağlantısı çözülemeyen hammaddede tüketilen `—`** (v2 `product_trees` seyrek; `material_code_id` ya da Hammadde tipli düğüm `product_code_id` üzerinden çözülür, kökten çarpan `unit_quantity` çarpımı). **Bölüm 2 — Sipariş Bazında Üretim Akışı (WIP):** solda sipariş listesi (seçili accent zemin + sol çubuk, closure'da korunur), sağda seçili siparişin `routes` sequence sırasına göre aşama zinciri (numaralı işaretçi + dikey çizgi). üretilen = o operasyonda (`work_orders.operation_id`, aynı sipariş) `production` toplamı; aktarılan = sonraki aşamanın üretileni; ara stok = üretilen − aktarılan (negatif danger). Son aşama: `stok.stageConsumeLast`/`stok.stageFinished`. `036` migration'ı GEREKMEDİ — gerekli sütunlar `005_product_codes`'ta zaten var (adlar: `material_weight`/`material_length`/`supply_days`/`outgoing_operation_id`/`parent_product_code`). Yeni `stok.*` + `menu.stok` anahtarları.

**Tedarikçi & Site Yönetimi (yeni modül + migration + ETL + BE, `docs/tedarikci-site-brief.md`):** basit CRUD; v2'de yoktu (kapsam kararında elenmişti, geri alındı). Sol menüde "Tedarikçi & Kalite" grubunun **başında**. Sıralanabilir tablo — kolon başlığına tıklayınca sıralar, tekrar tıklayınca yön değişir (aktif kolonda ok), `localeCompare(…, "tr", { numeric: true })`. Arama (supplier/trigoRE/sqe/sqm/country/city/siteCode) + sayaç. SQE/SQM hücreleri iki satırlı (ad + `mailto:` e-posta). Form ortak `core/drawer.js` ile; zorunlu alan yalnız **supplier**. **Not:** tasarımdaki "VİNFAST SQE/SQM" müşteriye özel — sözlükte `SQE`/`SQM` bırakıldı, müşteri adı gömülmedi. **Migration `037_sites`** (ev stili; `site_code`'da tekillik YOK — "1" gibi çakışabilir değerler var; tekil anahtar yalnız `(tenant_id, legacy_id)`). ETL: `sites` → doğrudan camelCase→snake_case. BE: `Site` Dto + Repository + Validator (+ ortak `Validator::email()` eklendi, boş geçerli) + Controller + `sites` routing; optimistic lock `updated_at`. **SONRASI (ayrı iş):** bu modül oturduğuna göre Genel Bakış'a "Tedarikçi Dağılımı — Ülke" bölümü eklenebilir. Yeni `site.*` + `menu.sites` anahtarları.

**Kalite Kontrol (yeni ekran + migration + ETL + BE, `docs/kalite-kontrol-brief.md`, referans v78 `viewKalite`):** sipariş bazında kontrol planı; solda kontrol planı OLAN siparişler (ürünü için `control_plans` tanımlı olanlar), sağda seçili siparişin plan maddeleri **sıra + operasyon** bazında gruplu (`G. Hammadde Kabul`, `1. Kesim`). Ölçüsel maddeye değer, nitel maddeye Uygun/Uygun Değil. Sonuç ölçüselde OTOMATİK (`alt ≤ değer ≤ üst`). Sağ üst **vardiya** seçici (1/2/3, aktif `--color-accent-900`). Ölçüsel maddede kumpas görselleştirmesi (nominal işareti sabit, değer işaretçisi `(değer−alt)/(üst−alt)` konumda, uygunsuzsa danger). **APPEND-ONLY:** her giriş `quality_measurements`'a YENİ satır (`measured_at` bugün, `shift` seçili vardiya); ekranda (sipariş, madde) için EN SON ölçüm (en yüksek id), geçmiş korunur. Satır içi anlık sessiz kayıt; hata → toast. Sipariş no `#orders?id=` çapraz bağlantı. **Migration `036_quality_control`:** `control_plans` + `quality_measurements` (ev stili: tenant FK/index, `IF NOT EXISTS`, `schema_migrations`). `sequence_label VARCHAR(8)` (metin "G"/"S"/"1".."5" — `routes.sequence` DECIMAL(5,1) ile KARIŞTIRILMAZ). **`sample_size VARCHAR(128)`** (brief'te INT ama `numuneAdedi` 124'ün 120'sinde serbest metin — "FR-09 Ölçüm Frekans Tablosu"). **`operation_label VARCHAR(128)`** eklendi (brief'te yoktu): ham `operasyon` metni grup başlığı için saklanır — çoğu değer rota-dışı (`operation_id` NULL). ETL: `kontrolPlani`→`control_plans` (ürün kod eşleşmesi, yoksa atla; operasyon/iş merkezi MEVCUT kayda eşlenir yoksa NULL, oto-oluşturma yok), `kaliteOlcumleri`→`quality_measurements` (orderId + kontrolPlaniId legacy haritasından; biri çözülemezse atla). BE: `ControlPlan`/`QualityMeasurement` Dto + Repository + Controller (control-plans salt okunur index/show; quality-measurements index/show/store append-only, optimistic lock YOK). **Melih'e sorulacak:** aynı (sipariş, madde) için çok ölçüm var (ölçüm günlüğü) — şimdilik son değer gösterilir, yeni giriş satır ekler (brief seçenek 1). Yeni `kk.*` + `menu.kalite-kontrol` anahtarları.

**Üretim Siparişleri v2 (yenileme, `docs/uretim-siparisleri-brief.md`, referans v78 `viewOrders`):** planlamacı ekranı; tüm siparişler. Asıl yenilik **İş Emri Aç** akışı. **Durum kararı:** `orders.status` 9-aşamalı akış (`Order::STATUSES` / `/order-statuses`) **korunur, backend değişmez** (karar: 9-aşamayı koru). Tasarımın 3'lü modeli uyarlandı: **"Aktif" = kapanmamış sipariş** (`status ∉ {Tamamlandı, İptal}`) — KPI/filtre bunu kullanır; **"Durduruldu" kavramı yok** (gerekirse İptal). Tabloda **İKİ AYRI sütun**: **DURUM** (saklanan 9-aşama, ton haritalı rozet) ve **RİSK** (hesaplanan: Kapasite Yetersiz / Termin Riski / İş Emri Bekliyor; risk yoksa boş) — hesaplanan rozet saklanan durumun üstüne binmez, yanında durur. 4 KPI (Aktif/İş Emri Bekliyor/Termin Riski/Açık İş Emri). Sıralanabilir tablo + arama + filtreler (Tümü/Aktif/İş Emri Bekliyor/Termin Riski/Tamamlandı). İlerleme SON rota adımından; ETA `core/eta.js`; Kapasite Yetersiz `core/bottleneck.js:productBottleneck`. **İş Emri Aç modalı:** rota `sequence` gruplarına göre; her adımda aktif hat ön-seçili, birden çok makine seçeneğinde "İkinci makine ekle" ile bölme, pay = `getCapacity` orantılı (kapasite tanımsızsa eşit böl + uyarı), varyant seçimi (`route_variants`). Kaydetme: her (sıra, makine) için bir `work_orders` (`wo_no={orderNo}-{sıra}`, bölünmüşse `split_label` A/B — varyant seçiliyse split_label'a eklenir; `status='Aktif'`; hedef paya göre yuvarlanır, artık son makineye). **Toplu BE ucu yok** → sıralı yazılır, hata olursa açılanlar geri silinir (telafi = hepsi ya da hiçbiri). Rota yoksa toast, modal açılmaz. Zaten iş emri varsa yeniden-açma onayı. Form modalı (canlı rota önizlemesi; kaynak uretim/stok; durum 9-aşama listesi). Silme: bağlı iş emri varsa engellenir. Çapraz: `İş Emirlerini Görüntüle → #work-orders?id=`; `focusId` (#orders?id=) korunur. **Temizlik:** `lookups.js` eski `ORDER_STATUS_OPTIONS` (Aktif/Tamamlandı/İptal) aslında **İŞ EMRİ** durum listesiydi (yanıltıcı ad) → `WORK_ORDER_STATUS_OPTIONS` olarak yeniden adlandırıldı (`workOrders.js` güncellendi); sipariş durumu için tek kaynak `/order-statuses`. Yeni `uo.*` anahtarları (brief `us.*` diyordu ama o önek Kullanıcılar'da — çakışma önlendi). Not: iş emri durumu (`work_orders.status`) 3 durumdur ve sipariş durumundan ayrıdır.

**Satış Siparişleri (yeni ekran, `docs/satis-siparisleri-brief.md`, referans v78 `viewSatisSiparisleri`):** aynı `orders` tablosunu **`source='satis'`** görünümüyle okur (Üretim Siparişleri ile aynı kayıtlar, farklı odak); migration yok. Sıralanabilir tablo (kolon başlığı) + arama (satış no/takip no/müşteri/ürün/not) + durum filtreleri (Tümü/İş Emri Bekliyor/Termin Riski/Tamamlandı) + sayaç. **Üretim durumu HESAPLANIR** (elle `orders.status` DEĞİL — ekranda gösterilmez): ilerleme yalnız **son rota adımından** (ürünün `routes` en yüksek `sequence`'i; aynı sırada bölünmüş iş emirleri toplanır). Durum rozeti: iş emri yok→İş Emri Bekliyor (warning), son adım hedefe ulaştı→Tamamlandı (success), tahmini bitiş>teslim→Termin Riski (danger), diğer→Üretimde (accent). Termin riski `core/eta.js:estimateCompletion` (Genel Bakış ile ortak). **Rapor modalı:** üst bilgi + TAHMİNİ BİTİŞ KPI'ları (üretilen/hedef/kalan/eta; iş emri yoksa uyarı) + bölünmüş son adım tablosu (her WO ayrı ETA, siparişin bitişi en geçi) + tüm iş emirleri (rota sırası, ilerleme). Drawer (yeni/düzenle): satış no, müşteri, ürün(FK), hedef, sipariş/teslim tarihi, not — kaydederken **`source='satis'` + başlangıç durumu + üretilen takip no (`SIP-yıl-sıra`)** otomatik yazılır. Eylemler: Rapor · Üretim Siparişinde Gör (`#orders?id=`) · Düzenle. Menü: **`Siparişler` → `Üretim Siparişleri`** olarak yeniden adlandırıldı; yeni **Satış Siparişleri** Satış grubunun başına. Yeni `ss.*` + `menu.satis-siparisleri` anahtarları.
...
**Genel Bakış v2 (yenileme, `docs/genel-bakis-brief.md`, tasarım `Genel-Bakis-v2.dc.html`):** SALT OKUNUR pano; veri istemcide türetilir (`listAll` — GET `/dashboard` artık çağrılmaz, `DashboardRepository`/`overview()` dokunulmadı, dışarıdan çağıran olabilir diye bırakıldı). **Kapsam dışı** (modül yok): Tedarikçi Sitesi + Parça KPI'ları ve Tedarikçi Dağılımı—Ülke bölümü; yerlerine Aktif Sipariş + Açık İş Emri KPI'ları (kod yorumunda `sites`/`parts` gelince geri konacağı not düşüldü). **5 KPI:** aktif sipariş (İptal/Tamamlandı olmayan), açık iş emri (Aktif), geciken görev (durum≠Tamamlandı & termin<bugün), görev tamamlama (Tamamlandı/toplam %), kapasite uyarısı (`computeDataWarnings().length`). **Geciken Görevler** tablosu (görev/sorumlu/termin/gecikme, en çok gecikenden aza, ilk 6 + "+N görev daha"). **Ürün Bazlı Darboğaz** (en kısıtlı 8; `productBottleneck` kapasitesi, çubuk `cap/max`, oran<%15 danger). **Üretim Takibi (MRP)** özeti: aktif sipariş + bugünkü üretim; termin riski taşıyan siparişler — risk sebebi üç tür: **Termin gerisinde — N gün** (ETA>teslim), **Kapasite yetersiz** (kalan/günlük kap > kalan gün), **Hammadde eksiği** (Stok Durumu net stok negatif). Çapraz bağlantılar: görev→`#tasks`, ürün→`#product-codes?id=`, sipariş→`#orders?id=`, darboğaz→`#capacities?id=`. **Ortak yardımcılar çıkarıldı** (brief Sıra 1): `core/bottleneck.js` (`createCapacityHelpers` → `getCapacity`/`productBottleneck`/`computeDataWarnings` + `groupBySeq`) ve `core/eta.js` (`estimateCompletion` — son 7 üretim ort., v78 `workOrderStats`); `capacities.js` bunlara **davranışı değişmeden** bağlandı (tek fark: net dakika ortak `core/capacity.js:netWorkMinutes`'ten, csOzet(wh).total ile aynı). Yeni `gb.*` anahtarları; migration YOK (tüm alanlar mevcut).
